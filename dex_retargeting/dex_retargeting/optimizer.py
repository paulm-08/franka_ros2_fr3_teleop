...
class DexPilotOptimizerAdapted(Optimizer):
    """Retargeting optimizer using the method proposed in DexPilot
    ...
    The DexPilotOptimizerAdapted class modifies the original DexPilot approach by employing a progressive projection
    distance function. This function dynamically adjusts the projection distance and the weights based on the proximity
    of the finger tips, enhancing the adaptability and effectiveness of the retargeting process. It also enforces a 
    pinching constraint between the thumb and index finger.
    ...
    retargeting_type = "DEXPILOTADAPTED"
    ...
    def get_objective_function(
        ...
        # Compute final reference vector
        reference_vec = np.where(
            self.projected[:, None], projected_vec, normed_vec[:len_proj]
        )  # (6, 3)
        # reference_vec = normed_vec[:len_proj]

        reference_vec = np.concatenate(
            [reference_vec, normed_vec[len_proj:]], axis=0
        )  # (10, 3)
        torch_target_vec = torch.as_tensor(reference_vec, dtype=torch.float32)
        torch_target_vec.requires_grad_(False)
        ...
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            # ---- MOD ----
            normal_link_names = ["thumb_normal","index_normal"]
            normal_indices = self.get_link_indices(normal_link_names)
            normal_poses = [
                self.robot.get_link_pose(index) for index in normal_indices
            ]
            normal_pos = np.array(
                [pose[:3, 3] for pose in normal_poses]
            )
            ...
            self.body_pos = np.concatenate(
                [self.body_pos, normal_pos, side_pos], axis=0
            )
            ...
            normal_indices_local = [len(self.computed_link_indices) + i for i in range(len(normal_indices))]
            side_indices_local = [len(self.computed_link_indices) +len(normal_indices) + i for i in range(len(side_indices))]
            ...
            normal_vec = torch_body_pos[normal_indices_local,:] - torch_body_pos[tip_indices_local, :]            # Normalize the normal vectors
            normed_normal_vec = torch.nn.functional.normalize(normal_vec, dim=1)
            ...
            if grad.size > 0:
                ...
                # ---- MOD: Append normal jacobians manually ----
                for normal_idx in normal_indices:
                    normal_body_jacobian = self.robot.compute_single_link_local_jacobian(
                        qpos, normal_idx
                    )[:3, ...]
                    normal_pose = self.robot.get_link_pose(normal_idx)
                    normal_rot = normal_pose[:3, :3]
                    normal_jacobian = normal_rot @ normal_body_jacobian
                    jacobians.append(normal_jacobian)
                # --------------------------------------------
