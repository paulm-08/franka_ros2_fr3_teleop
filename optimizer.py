...
class Optimizer:
    ...
    def __init__(
        ...
        for target_joint_name in target_joint_names:
            ...
            # if target_joint_name not in ["15","3"]: # Ignore the thumb and index finger
...
class DexPilotOptimizer(Optimizer):
    ...
    def get_objective_function(
        ...
        print("Projection distance:", self.project_dist)
        print("Escape distance:", self.escape_dist)
        ...
        # --- MOD ---
        # Adaptive weight for projected distance
        def adaptive_weight(d, min_d=0.03, max_d=self.escape_dist, w_min=1.0, w_max=200.0):
            # Inverse distance weighting (can also try exponential or sigmoid)
            d_clipped = np.clip(d, min_d, max_d)
            return w_min + (max_d - d_clipped) / (max_d - min_d) * (w_max - w_min)
        
        weight = np.where(
            self.projected,
            adaptive_weight(target_vec_dist[:len_proj]),
            normal_weight
        )
        ...
        # reference_vec = normal_vec[:len_proj]
        ...
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            self.body_pos = np.array([pose[:3, 3] for pose in target_link_poses])


            # # Torch computation for accurate loss and grad
            # torch_body_pos = torch.as_tensor(self.body_pos)
            # torch_body_pos.requires_grad_()
            ...
            # ---- MOD ----
            forward_link_names = ["thumb_forward","index_forward"]
            forward_indices = self.get_link_indices(forward_link_names)
            forward_poses = [
                self.robot.get_link_pose(index) for index in forward_indices
            ]
            forward_pos = np.array(
                [pose[:3, 3] for pose in forward_poses]
            )
            side_link_names = ["thumb_side","index_side"]
            side_indices = self.get_link_indices(side_link_names)
            side_poses = [
                self.robot.get_link_pose(index) for index in side_indices
            ]
            side_pos = np.array(
                [pose[:3, 3] for pose in side_poses]
            )

            self.body_pos = np.concatenate(
                [self.body_pos, forward_pos, side_pos], axis=0
            )

            # Torch computation for accurate loss and grad
            torch_body_pos = torch.tensor(self.body_pos, dtype=torch.float32, requires_grad=True)
            # torch_body_pos.requires_grad_()
            ...
            forward_indices_local = [len(self.computed_link_indices) + i for i in range(len(forward_indices))]
            side_indices_local = [len(self.computed_link_indices) +len(forward_indices) + i for i in range(len(side_indices))]
            tip_indices_global = self.get_link_indices(["thumb_tip_head", "index_tip_head"])

            # Map global indices to local indices in computed_link_indices
            tip_indices_local = []
            for i in tip_indices_global:
                if i in self.computed_link_indices:
                    tip_indices_local.append(self.computed_link_indices.index(i))
                else:
                    raise ValueError(f"Tip link index {i} not in computed_link_indices")

            # Now use these local indices to slice torch_body_pos
            forward_vec = torch_body_pos[forward_indices_local,:] - torch_body_pos[tip_indices_local, :]            # Normalize the forward vectors
            normed_forward_vec = torch.nn.functional.normalize(forward_vec, dim=1)
            ...
            cos_sim_forward = torch.sum(normed_forward_vec[0] * normed_forward_vec[1])
            cos_sim_side = torch.sum(normed_side_vec[0] * normed_side_vec[1])

            forward_weight = 0.2  # Weight for the forward parallelism loss
            side_weight = 0 # Weight for the side parallelism loss

            # Turn into a loss: penalize deviation from 1 (parallel)
            if self.projected[0]:
                self.forward_parallel_loss = (1.0 - cos_sim_forward)**2 * forward_weight
                self.side_parallel_loss = (1.0 - cos_sim_side)**2 * side_weight
            else:
                self.forward_parallel_loss = (1.0 - cos_sim_forward)**2 * 0.0
                self.side_parallel_loss = (1.0 - cos_sim_side)**2 * 0.0

            # print("Forward vecs:", forward_vec.detach().numpy())
            # print("Side vecs:", side_vec.detach().numpy())
            # print("cos_sim_forward:", cos_sim_forward.item())
            ...
            self.huber_distance = huber_distance.sum()
            # result = huber_distance.cpu().detach().item()


            # ---- MOD ----
            # Add the forward and side parallelism loss to the total loss
            result = self.huber_distance + self.forward_parallel_loss + self.side_parallel_loss
            # ---- MOD ----
            ...
            if grad.size > 0:
                ...
                # ---- MOD: Append forward jacobians manually ----
                for forward_idx in forward_indices:
                    forward_body_jacobian = self.robot.compute_single_link_local_jacobian(
                        qpos, forward_idx
                    )[:3, ...]
                    forward_pose = self.robot.get_link_pose(forward_idx)
                    forward_rot = forward_pose[:3, :3]
                    forward_jacobian = forward_rot @ forward_body_jacobian
                    jacobians.append(forward_jacobian)
                # --------------------------------------------
                # ---- MOD: Append side jacobians manually ----
                for side_idx in side_indices:
                    side_body_jacobian = self.robot.compute_single_link_local_jacobian(
                        qpos, side_idx
                    )[:3, ...]
                    side_pose = self.robot.get_link_pose(side_idx)
                    side_rot = side_pose[:3, :3]
                    side_jacobian = side_rot @ side_body_jacobian
                    jacobians.append(side_jacobian)
                # --------------------------------------------
                ...
                grad_qpos = np.matmul(grad_pos, np.array(jacobians))
                # grad_qpos = np.array([
                #     grad_pos[i].dot(jacobians[i]) for i in range(len(grad_pos))
                # ]).sum(axis=0)
                ...
                # Safety check
                if not np.all(np.isfinite(grad_qpos)):
                    raise ValueError("Gradient has NaNs or infs")
                if grad_qpos.shape != grad.shape:
                    raise ValueError(f"Gradient shape mismatch: got {grad_qpos.shape}, expected {grad.shape}")

                grad[:] = grad_qpos[:]

            return result.cpu().detach().item()
            # return result
